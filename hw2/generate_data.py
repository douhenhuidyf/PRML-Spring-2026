...
split_index = np.random.permutation(np.arange(X.shape[0]))
size = int(0.5 * X.shape[0])
train_x, train_y = X[split_index[:size]], labels[split_index[:size]]
test_x, test_y = X[split_index[size:]], labels[split_index[size:]]

np.savez('moons_3d.npz',
         train_x=train_x,
         train_y=train_y,
         test_x=test_x,
         test_y=test_y)
